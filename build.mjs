...
await Promise.all([
  ...
  build({
    ...base,
    entryPoints: ['src/preload/auth.ts'],
    outdir: 'build/preload',
    platform: 'node',
    target: 'node20',
    format: 'cjs',
  ...
  build({
    ...base,
    entryPoints: ['src/preload/tab.ts'],
    outdir: 'build/preload',
    platform: 'node',
    target: 'node20',
    format: 'cjs',
  ...
  build({
    ...base,
    entryPoints: ['src/renderer/auth.ts'],
    outdir: 'build/renderer',
    platform: 'browser',
    target: 'chrome124',
    format: 'iife',
...
copyFileSync('src/renderer/auth.html', 'build/renderer/auth.html')
